...
import { getBlogContraryTypeMap,getBlogType,updateBlog,getBlogById,getBlogTypeMap } from '@/api/index' //获取文章分类信息 id:name键值对
...
export default {
  ...
  data(){
    return {
      ...
      sizeForm: { //文章选择区
        ...
        type: '',
        types: [],  //所有分类信息
        typeMap: {},  //map集合，里面有分类信息的 名称:id
        idNameMap: {},  //map集合，里面有分类信息的 id:名称 
      },
  ...
  methods:{
    getMessage(){ //通过传来的 文章id 找到文章信息
      let _this = this;
      getBlogTypeMap().then((response) =>{  // 获取到博客分类的 id:name
        if(response.code === 20000){
          _this.sizeForm.idNameMap = response.data;
        } 
      })
      getBlogById(this.$route.params.blogId).then((response) =>{
        if(response.code === 20000){
          _this.notify("文章获取成功","success");
          _this.sizeForm.blogTitle = response.data.data.blogTitle;
          _this.sizeForm.blogRemark = response.data.data.blogRemark;
          _this.sizeForm.blogImage = response.data.data.blogImage;
          _this.sizeForm.type = _this.sizeForm.idNameMap[response.data.data.blogType];
          _this.inputCont = response.data.data.blogContent;
          _this.sizeForm.blogContentImg = response.data.data.blogContentImg;
        }
      })
    },
    ...
    onSubmit() {  //提交
      ...
      params.blogId = this.$route.params.blogId;  //后端要使用 updateById()方法来更新数据，所以blog对象把id也传过去
  ...
  mounted(){
    ...
    this.getMessage();  //进入编辑页面后先通过传来的 文章id 找到文章信息
